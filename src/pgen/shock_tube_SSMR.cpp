...
//========================================================================================
//! \fn Real SmoothSMR(Real x, RegionSize rs)
//! \brief Mesh generator function 
//! Creates smooth transition region between two regions of different resolution
//========================================================================================

Real SmoothSMR(Real x, RegionSize rs);
...
Real SmoothSMR(Real x, RegionSize rs){
  ...
  Real t = 2.0*x-1.0;
  Real x1 = N0/NT;
  Real x2 = 2*((rs.x1max/(NT*dxBase) - k)/(1-k) - x1/2);
  if ((x1 <0) || (x1>1)) {
    msg << "### FATAL ERROR in function [SmoothSMR]"
          << std::endl << "Invalid x1 value " << x1 <<std::endl;
      ATHENA_ERROR(msg);
  }
  if ((x2 <x1) || (x2>1)) {
    msg << "### FATAL ERROR in function [SmoothSMR]"
          << std::endl << "Invalid x2 value " << x2 <<std::endl;
      ATHENA_ERROR(msg);
  }

  Real a = 1/(2*(x2 + 0.5*(k-1)*(x2-x1) + k*(1-x2)));
  Real w = 0.0;
  if ((abs(t) > x1) && (abs(t) < x2) ){
    w = abs(t) + (k-1)*0.5*SQR(abs(t)-x1)/(x2-x1);
  } else if (abs(t) >= x2) {
    w = x2 + (k-1)*0.5*(x2-x1)+ k*(abs(t)-x2);
  } else if (abs(t) <= x1) {
    w = abs(t);
  } else {
    msg << "### FATAL ERROR in function [SmoothSMR]"
          << std::endl << "Invalid x value entered" <<std::endl;
      ATHENA_ERROR(msg);
  }
  if (t < 0.0) {
    w *= -1;
  ...
  w += 0.5;
  //std::cout << w << std::endl;
  return w*rs.x1max+(1.0-w) * rs.x1min;
}


//========================================================================================
//! \fn void Mesh::InitUserMeshData(ParameterInput *pin)
//! \brief Enroll User mesh generator function
//========================================================================================

void Mesh::InitUserMeshData(ParameterInput *pin){
  Real x1rat = pin->GetOrAddReal("mesh","x1rat",0);
  if (x1rat == -1) {
    dxBase = pin->GetReal("problem","dxBase");
    k = pin->GetInteger("problem","k");
    N0 = pin->GetInteger("problem","nx1high");
    NT = pin->GetInteger("mesh","nx1") / 2;
    std::cout << NT << std::endl;
    EnrollUserMeshGenerator(X1DIR, SmoothSMR);
  }
  return;
}
